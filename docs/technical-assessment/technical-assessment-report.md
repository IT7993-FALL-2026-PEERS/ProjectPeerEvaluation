...
12. Next steps
M1 to complete `docs/architecture/system-architecture.md` and
`docs/requirements/critical-workflows.md`; both are folded into sections 1 and 10 on
arrival.
M3 to complete `docs/requirements/requirements.md` (required functionality and
acceptance criteria) in the week of 21 Sep, then the Requirements Traceability Matrix
skeleton in the week of 28 Sep.
This report to be reviewed with the sponsor before implementation work begins, as the
specification requires.
Defects to be triaged with the sponsor at the 28 Sep review: which are in scope for
